...
export class TransportBot extends Bot {
  ...
  private fillLinks(bot: Creep) {
    const links = Object.keys(Memory.rooms[bot.memory.room].monitoring.structures.links).map(linkId =>
    ...
    const linkToFill = linksToSend.sort(
        ...
        if (bot.memory.params.pickup != null) {
          if (bot.memory.params.pickup === "loot") {
            if (bot.pos.roomName !== bot.memory.room) {
              const droppedEnergy = Object.entries(
                Memory.rooms[bot.memory.room].monitoring.resources.droppedResources
              ).sort(([, droppedResourceA], [, droppedResourceB]) => droppedResourceB.amount - droppedResourceA.amount);
